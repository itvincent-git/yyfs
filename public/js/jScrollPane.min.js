var isScrolling=false;(function($){$.jScrollPane={active:[]};$.fn.jScrollPane=function(settings)
...
isScrolling=true;changeScroll();initDrag();dragMiddle=getPos(event,'Y')-dragPosition-currentOffset.top;$('html').bind('mouseup',onStopDrag).bind('mousemove',updateScroll).bind('mouseleave',onStopDrag)
if($.browser.msie){$('html').bind('dragstart',ignoreNativeDrag).bind('selectstart',ignoreNativeDrag);}
return false;};var onStopDrag=function()
{if($.browser.msie){$drag[0].releaseCapture();}
isScrolling=false;recoverScroll();$('html').unbind('mouseup',onStopDrag).unbind('mousemove',updateScroll);dragMiddle=percentInView*paneHeight/2;if($.browser.msie){$('html').unbind('dragstart',ignoreNativeDrag).unbind('selectstart',ignoreNativeDrag);}};var positionDrag=function(destY)
...
{positionDrag(getPos(e,'Y')-currentOffset.top-dragMiddle);};var dragH=Math.max(Math.min(percentInView*(paneHeight-settings.arrowSize*2),settings.dragMaxHeight),settings.dragMinHeight);$drag.css({'height':dragH+'px'}).bind('mousedown',onStartDrag);$drag.mouseenter(function(){changeScroll()});$drag.mouseleave(function(){recoverScroll()});var oldDragImg="";var oldDragTopImg="";var oldDragBtmImg="";function changeScroll(){$drag.addClass("jScrollPaneDragHover");$drag.find(".jScrollPaneDragTop").addClass("jScrollPaneDragTopHover");$drag.find(".jScrollPaneDragBottom").addClass("jScrollPaneDragBottomHover")}
function recoverScroll(){if(!isScrolling){$drag.removeClass("jScrollPaneDragHover");$drag.find(".jScrollPaneDragTop").removeClass("jScrollPaneDragTopHover");$drag.find(".jScrollPaneDragBottom").removeClass("jScrollPaneDragBottomHover")}}
var trackScrollInterval;var trackScrollInc;var trackScrollMousePos;var doTrackScroll=function()
